...
defimpl Plsm.Database, for: Plsm.Database.MySql do
  ...
  defp get_type(start_type) do
    ...
    cond do
      String.starts_with?(downcase, "int") ->
        :integer

      String.starts_with?(downcase, "bigint") ->
        :integer

      String.starts_with?(downcase, "tinyint(1)") ->
        :boolean

      String.starts_with?(downcase, "tinyint") ->
        :integer

      String.starts_with?(downcase, "bit") ->
        :integer

      String.starts_with?(downcase, "varchar") ->
        :string

      String.contains?(downcase, "char") ->
        :text

      String.starts_with?(downcase, "text") ->
        :string

      String.starts_with?(downcase, "mediumtext") ->
        :string

      String.starts_with?(downcase, "float") ->
        :float

      String.starts_with?(downcase, "double") ->
        :float

      String.starts_with?(downcase, "decimal") ->
        :decimal

      String.starts_with?(downcase, "date") ->
        :date

      String.starts_with?(downcase, "datetime") ->
        :date

      String.starts_with?(downcase, "timestamp") ->
        :date
