defmodule Plsm.IO.Export do
  ...
  def type_output(field) do
    case field do
      {name, type} when type == :integer ->
        four_space("field :#{name}, :integer\n")

      {name, type} when type == :decimal ->
        four_space("field :#{name}, :decimal\n")

      {name, type} when type == :float ->
        four_space("field :#{name}, :float\n")

      {name, type} when type == :string ->
        four_space("field :#{name}, :string\n")

      {name, type} when type == :text ->
        four_space("field :#{name}, :string\n")

      {name, type} when type == :date ->
        four_space("field :#{name}, :naive_datetime\n")

      {name, type} when type == :boolean ->
        four_space("field :#{name}, :boolean\n")
  ...
  defp remove_primary_key(columns) do
    Enum.filter(columns, fn column -> column.primary_key == false end)
  end
